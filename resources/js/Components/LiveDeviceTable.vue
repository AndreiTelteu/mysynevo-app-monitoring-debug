...
function saveNickname(device, event) {
    const value = event.target.value.trim();
    event.target.value = device.nickname || '';
    if (value === (device.nickname || '')) return;
    emit('update-nickname', device, value);
}
...
<template>
    <section class="device-section">
        ...
        <div class="table-wrap">
            <table>
                <thead><tr><th>Actions</th><th>Device</th><th>Presence</th><th>App</th><th>Network</th><th>Environment / route</th><th>Salesforce Marketing Cloud</th><th>Last seen</th></tr></thead>
                <tbody>
                    <tr v-for="device in devices" :key="device.id">
                        <td class="actions-cell"><button class="pin-button" :class="{ active: device.isPinned }" type="button" :aria-label="device.isPinned ? 'Unpin device' : 'Pin device'" @click="$emit('toggle-pin', device)">{{ device.isPinned ? 'PINNED' : 'PIN' }}</button><button class="hide-button" :class="{ active: device.isHidden }" type="button" :aria-label="device.isHidden ? 'Unhide device' : 'Hide device'" @click="$emit('toggle-hidden', device)">{{ device.isHidden ? 'UNHIDE' : 'HIDE' }}</button></td>
                        <td><input class="nickname-input" :value="device.nickname || ''" :placeholder="displayName(device)" type="text" aria-label="Device nickname" @blur="saveNickname(device, $event)" /><small>{{ device.device.manufacturer || 'Unknown maker' }} · {{ device.device.osVersion || 'Unknown OS' }}</small><code>{{ device.deviceId }}</code></td>
