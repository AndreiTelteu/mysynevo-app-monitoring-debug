...
<script>
const DeviceTable = {
    props: { title: String, devices: Array, isOnline: Function, displayName: Function, elapsed: Function },
    emits: ['toggle-pin'],
    template: `
        <section class="device-section">
            <div class="section-heading"><div><p class="eyebrow">{{ devices.length }} device{{ devices.length === 1 ? '' : 's' }}</p><h2>{{ title }}</h2></div><span class="stream-label">LIVE SNAPSHOTS</span></div>
...
export default { components: { DeviceTable } };
</script>
...
.device-section { max-width: 1400px; margin: 0 auto 4rem; }.section-heading { display: flex; align-items: end; justify-content: space-between; gap: 1rem; margin-bottom: 1rem; }.section-heading h2 { margin: 0; font: 700 2rem/1 'Fraunces'; letter-spacing: -.04em; }.stream-label { color: var(--muted); font: .62rem 'DM Mono'; letter-spacing: .1em; }.table-wrap { overflow-x: auto; border: 1px solid var(--ink); background: rgba(255,253,246,.82); box-shadow: 7px 7px 0 rgba(20,35,33,.11); } table { width: 100%; min-width: 1100px; border-collapse: collapse; } th { padding: .8rem 1rem; border-bottom: 1px solid var(--ink); background: #e9e7dc; text-align: left; font: 500 .62rem 'DM Mono'; letter-spacing: .09em; text-transform: uppercase; } td { padding: 1rem; vertical-align: top; border-bottom: 1px solid var(--line); font-size: .8rem; } tbody tr:last-child td { border: 0; } tbody tr:hover { background: #edfad0; } td strong, td small, td code { display: block; } td strong { margin-bottom: .25rem; font-size: .82rem; } td small { color: var(--muted); font-size: .69rem; line-height: 1.4; } td code { max-width: 12rem; margin-top: .45rem; overflow: hidden; color: #9b4e36; font: .61rem 'DM Mono'; text-overflow: ellipsis; white-space: nowrap; }.url,.route { max-width: 14rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.route { color: #325d57; font: .69rem 'DM Mono'; }.presence { display: flex; align-items: center; gap: .35rem; font-weight: 700; }.presence i { width: .5rem; height: .5rem; border-radius: 50%; }.presence.online { color: #167342; }.presence.online i { background: #38bb67; box-shadow: 0 0 0 3px rgba(56,187,103,.15); }.presence.offline { color: #a34f42; }.presence.offline i { background: #ca7868; }.presence + small { margin: .4rem 0 0 .85rem; }.pin-button { border: 1px solid var(--ink); background: transparent; padding: .35rem .45rem; color: var(--ink); cursor: pointer; font: .6rem 'DM Mono'; }.pin-button.active { background: var(--lime); box-shadow: 2px 2px 0 var(--ink); }.empty-state { padding: 3rem; color: var(--muted); text-align: center; font: .8rem 'DM Mono'; }
